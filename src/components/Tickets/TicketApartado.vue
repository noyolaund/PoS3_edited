  ...
  <v-layout class="ticket" wrap row>
    ...
    <v-flex xs12>
      ...
      <div class="text-xs-center">
        <p>
          <strong>Ticket de apartado #{{ apartado.Numero }}</strong>
        </p>
        ...
        <br />
        <p>
          {{ apartado.Fecha | fechaExpresiva }}
        </p>
        <p>
          Lo atendió: <strong>{{ apartado.Usuario.Nombre }}</strong>
        </p>
        <p>
      ...
      <div class="text-xs-right">
        ...
        <p><strong>Su pago</strong> {{ apartado.Pago | currency }}</p>
        <p><strong>Anticipo</strong> {{ apartado.Anticipo | currency }}</p>
        <p>
          <strong>Cambio</strong>
          {{ (apartado.Pago - apartado.Anticipo) | currency }}
        </p>
        <p>
          <strong>Restante</strong>
          {{
            (apartado.Total - apartado.Anticipo - apartado.Abonado) | currency
          }}
        </p>
        <br />
      </div>
      <div class="text-xs-justify">
        <p>
          <strong>Nota: </strong> no nos hacemos responsables de los articulos
          despues de la fecha de vencimiento:
          {{ apartado.FechaVencimiento | fechaSinHora }}
        </p>
      </div>

      <div class="text-xs-center">
        <v-flex xs9 offset-xs1 class="text-xs-right con-borde-inferior">
          <br /><br />
        </v-flex>
      </div>
      <div class="text-xs-center">
        <strong class="text-xs-center">Firma del cliente</strong>
      </div>
      <div class="text-xs-center">
...
export default {
  ...
  beforeRouteUpdate(detallesRuta) {
    this.obtenerDetallesDeApartado(detallesRuta.params.idApartado);
  },
  beforeMount() {
  ...
  methods: {
    obtenerDetallesDeApartado(idApartado) {
      if (!idApartado) return this.$router.go(-1);
      this.cargando = true;
      HTTP_AUTH.get("ajustes/empresa")
        ...
        .then(() => {
          HTTP_AUTH.get(`apartado/${idApartado}`)
            .then((apartado) => {
