...
NOMBRE_DE_EJECUTABLE_SIN_EXTENSION = "sublime_pos_3_by_parzibyte_para_raspberry_pi"
NOMBRE_ARCHIVO_CHECKSUM = "checksum_archivos.go"

if input("Recuerda que ya debiste ejecutar npm run build y que la carpeta dist debe existir antes de ejecutar este script. Esto es porque en Raspbian el comando npm run build se queda bloqueado esperando por siempre. ¿Continuar? s/n: ") == "n":
    exit()
...
os.chdir(ruta_api)
nombre_ejecutable = f"{NOMBRE_DE_EJECUTABLE_SIN_EXTENSION}_{arquitectura}"
comando = f"go build -tags produccion -o {nombre_ejecutable}"
